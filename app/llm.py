...
2. Every factual claim carries an inline citation in the exact format (Company, p.N), placed immediately after the sentence it supports, where Company matches one of the company names given in the excerpts and N is the page number printed for that excerpt. If the excerpts include more than one fiscal year for that company, cite the year too, in the format (Company YYYY, p.N) — pull YYYY from the excerpt's own bracket label, never assume it.
...
# Matches "(Company, p.45)" / "(Company p.~45)" / "(Company 2025, p.45)" — the
# year group is captured (not just tolerated) so a citation can be checked
# against the specific fiscal year it names, which matters once excerpts
# span more than one year for the same company and "p.68" alone is ambiguous.
CITATION_RE = re.compile(
    r"\(([A-Z][\w&.\-]*(?:\s+[A-Z&][\w&.\-]*){0,3}),?\s*(?:(\d{4})\s*,?\s*)?p\.\s*~?(\d+)\)"
)
...
def _pages_by_company(chunks: list[dict]) -> dict[str, dict[str, set[int]]]:
    """company -> year -> set of retrieved pages. Keyed by year (not just
    company) so a citation naming a year can be checked against that exact
    fiscal year rather than any year the company happens to have on file."""
    pages: dict[str, dict[str, set[int]]] = {}
    for c in chunks:
        try:
            page = int(c["approx_page"])
        except (TypeError, ValueError):
            continue
        year = str(c.get("year") or "")
        pages.setdefault(c["company"], {}).setdefault(year, set()).add(page)
    return pages
...
def _resolve(raw_company: str, year: str | None, page: int, pages_by_company: dict[str, dict[str, set[int]]]):
    """Match a citation's company (and, if given, fiscal year) against
    retrieved chunk metadata, confirming the page is within 3 of a real
    retrieved page (approx_page is a chunking estimate, not an exact page).
    With no year named, any year retrieved for that company counts — the
    common case where only one year is in scope and naming it is optional."""
    ...
    candidate_pages: set[int] = years_pages.get(year, set()) if year else set().union(*years_pages.values())
    verified = any(abs(page - p) <= 3 for p in candidate_pages)
